...
$sql = "SELECT * FROM parcels WHERE status = '5'";
...
<div class="container">
    <div class="row min-vh-100 bg-light rounded justify-content-center mx-0 m-5">
        <div class="col-md-12 text-center">
            ...
            <div>
                <div>
                    <h2 class="mb-5 mt-4 text-danger">Returned Parcels List</h2>
                    <div class="mb-5 text-right">
                        <a href="dashboard.php"><button class="btn btn-danger">Dashboard</button></a>
                        <a href="returned.php"><button class="btn btn-danger">Back</button></a>
                    </div>
                        ...
                        <table class="table">
                            ...
                            <tbody>
                                ...
                                if (isset($_GET['search'])) {
                                    $search = $_GET['search'];
                                    $sql = "SELECT * FROM parcels WHERE track_id LIKE '%$search%' AND status = '5'";
                                } elseif (isset($_GET['city']) && isset($_GET['agent_name'])) {
                                    $city = $_GET['city'];
                                    $agent = $_GET['agent_name'];
                                    $sql = "SELECT * FROM parcels 
             WHERE sender_address LIKE '%$city%' 
             AND agent_name = '$agent' AND status = '5'";
                                } elseif (isset($_GET['city'])) {
                                    $city = $_GET['city'];
                                    $sql = "SELECT * FROM parcels 
             WHERE sender_address LIKE '%$city%' AND status = '5'";
                                } elseif (isset($_GET['agent_name'])) {
                                    $agent = $_GET['agent_name'];
                                    $sql = "SELECT * FROM parcels WHERE agent_name = '$agent' AND status = '5'";
                                } elseif (isset($_GET['from_date']) && isset($_GET['to_date'])) {
                                    $from_date = $_GET['from_date'];
                                    $to_date = $_GET['to_date'];
                                    $sql = "SELECT * FROM parcels WHERE date BETWEEN '$from_date' AND '$to_date' AND status = '5'";
                                } else {
                                    $sql = "SELECT * FROM parcels WHERE status = '5'";
                                }
                                $stmt = $conn->prepare($sql);
